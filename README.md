...
一个中文、移动端优先的企业 AI 转型事实审计网站。用户通过八项可核验记录获得变革深度、实际交付能力、运营证据和证据完整度，以及适应、重构、进化或革命的路线建议。
...
问卷版本为 `3.2`。八项记录包括：正式授权范围、真实运行流程数、有使用前后结果对比的 AI 应用数、过去 12 个月最近项目覆盖部门数、同一项目实际耗时、优先流程材料、一号位决策记录和已生效岗位变化。系统始终根据已知信息给出方向性路线建议；信息缺失或项目口径需要复核时，会降低置信度并显示风险提示。
