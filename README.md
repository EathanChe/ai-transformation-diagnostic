...
一个中文、移动端优先的企业 AI 转型路径测评网站。用户通过八个单维度问题获得变革深度、速度压力和组织承载三个分值，以及适应、重构、进化或革命的路线建议。
...
评分由共享规则模块完成。Pages Function 重新校验选项并分别计算变革深度、速度压力和组织承载；三项分值保持独立，不采用互补分数。模型 API 负责生成解释。模型未配置、超时或输出校验失败时，Function 会返回完整的规则模板报告。

问卷版本为 `2.0`。每题只测量一个变量：改变范围、审批层级、生产系统数量、业务影响时间、一号位参与频率、流程准备阶段、真实业务试运行周期和岗位职责调整范围。
